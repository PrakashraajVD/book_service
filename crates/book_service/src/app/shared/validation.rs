...
use garde::{
    I18n, Report, Validate,
    i18n::{InvalidCreditCard, InvalidPhoneNumber, InvalidUrl},
    with_i18n,
};
...
impl I18n for English {
    ...
    fn length_greater_than(&self, max: usize) -> Cow<'static, str> {
        match max {
            1 => Cow::Borrowed("Must not exceed character"),
    ...
    fn range_greater_than(&self, max: &dyn Display) -> Cow<'static, str> {
        format!("Must be lesser than or equal to {max}").into()
    }
    ...
    fn contains_missing(&self, pattern: &dyn Display) -> Cow<'static, str> {
        format!("Must contain the \"{pattern}\"").into()
    }
    ...
    fn email_invalid(&self, _reason: garde::i18n::InvalidEmail) -> Cow<'static, str> {
