...
use crate::{
    ...
    app::{
        ...
        shared::{Pagination, ValidatedJson, ValidationErrorResponse},
    },
...
pub async fn read(
    ...
    let book = Book::get_by_id(&mut state.db, &id)
        .await
        .map_err(|err| {
...
pub async fn update(
    ...
    .await
    .map_err(|err| {
        error!(target:"database","failed to updated: {err:?}");
        Error::DbUpdate
    })?;
...
pub async fn delete(
    ...
    Book::delete_by_id(&mut state.db, id)
        .await
        .map_err(|err| {
